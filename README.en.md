...
1. Create a directory for the project that you want to create (example:
   `temenin_isoman`), then open a Command Prompt (cmd) or Terminal in
   that directory.
...
5. Create a new Django project with this template.
   ...
   ```shell
   django-admin startproject --template="https://codeload.github.com/laymonage/django-template-heroku/zip/template" --extension="py,yml,md" --name="Procfile" temenin_isoman .
   ```

   > Note: rename `temenin_isoman` with the desired project name. Notice
...
3. Set the code coverage configuration on GitLab.
   ...
   command.
    ...
    ```shell
    coverage run --include="./*" --omit="venv/*,manage.py,temenin_isoman/*" manage.py test
    ```
